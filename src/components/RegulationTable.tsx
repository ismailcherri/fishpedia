  ...
  return (
    <div
      className={`rounded-2xl p-4 ring-1 ${
        active
          ? 'bg-water-50 ring-2 ring-water-500 dark:bg-water-900'
          : 'bg-white ring-water-100 dark:bg-water-900/50 dark:ring-water-800'
      }`}
    >
      ...
      <dl className="space-y-1.5 text-sm">
        <div className="flex justify-between gap-2">
          <dt className="text-slate-500 dark:text-slate-400">{t('minSize')}</dt>
        ...
        <div className="flex justify-between gap-2">
          <dt className="text-slate-500 dark:text-slate-400">{t('closedSeason')}</dt>
        ...
        {status === 'closed' && change && (
          <div className="flex justify-between gap-2 text-red-700 dark:text-red-300">
            <dt>{t('reopens')}</dt>
            <dd className="font-bold">{formatDate(change, lang)}</dd>
          </div>
        )}
...
export function RegulationTable({ fish }: { fish: FishSpecies }) {
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      ...
      <RegionRules region="brandenburg" rule={fish.regulations.brandenburg} />
    </div>
  )
}
