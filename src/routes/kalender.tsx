...
function CalendarPage() {
  ...
  return (
    <div>
      ...
      <p className="mb-4 mt-1 max-w-2xl text-sm text-slate-500 dark:text-slate-400">
        {t('calendarHint')}
      </p>

      <div className="overflow-x-auto rounded-2xl bg-white ring-1 ring-water-100 dark:bg-water-900 dark:ring-water-800">
        <table className="w-full min-w-[560px] border-collapse text-sm">
          <thead>
            <tr className="border-b border-water-100 dark:border-water-800">
              <th className="p-2 text-left font-bold">
                {t(region === 'berlin' ? 'regionBerlin' : 'regionBrandenburg')}
              </th>
              {MONTHS_DE.map((m, i) => (
                <th
                  key={i}
          ...
          <tbody>
            {regulated.map((fish) => {
              ...
              return (
                <tr
                  key={fish.id}
                  className="border-b border-water-50 last:border-0 dark:border-water-800/50"
                >
                  <td className="p-2">
                    <Link
                      to="/fisch/$slug"
                      params={{ slug: fish.id }}
                      className="font-semibold text-water-700 hover:underline dark:text-water-200"
                    >
                      {fish.names[lang]}
                    </Link>
                  </td>
                  {MONTHS_DE.map((_, i) => {
                    ...
                    const blocked = rule.fullyProtected
                      ? true
                      : rule.closedSeason
                        ? monthTouchesSeason(rule.closedSeason, month)
                        : false
                    return (
                      <td key={i} className="p-1 text-center">
                        <span
                          className={`mx-auto block h-5 w-5 rounded ${
                            ...
                            currentMonth === month
                              ? 'ring-2 ring-water-600 dark:ring-water-300'
                              : ''
                          }`}
                          role="img"
                          aria-label={blocked ? t('statusClosed') : t('statusOpen')}
                        />
                      </td>
                    )
                  })}
